...
const pointIcon = (color: string) => L.divIcon({
  className: '',
...
const hasCoords = (p?: RoutePoint | null): p is RoutePoint & { lat: number; lng: number } => p?.lat != null && p?.lng != null;

export interface OrderRouteMapProps {
  carico?: RoutePoint | null;
  scarico?: RoutePoint | null;
  garage?: RoutePoint | null;
  washStation?: RoutePoint | null;
...
export default function OrderRouteMap({ carico, scarico, garage, washStation, routePoints: roadPoints, height = 220 }: OrderRouteMapProps) {
  ...
  const allPoints: [number, number][] = roadPoints?.length ? [...roadPoints] : [...routePoints];
