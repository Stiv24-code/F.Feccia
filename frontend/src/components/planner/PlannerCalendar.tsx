...
type StatoColor = 'red' | 'yellow' | 'blue' | 'green' | 'gray';
...
const COLOR_BY_STATO: Record<string, StatoColor> = {
  PIANIFICABILE: 'red',
  PIANIFICATO: 'yellow',
  VIAGGIO: 'blue',
  CHIUSO: 'green',
...
const COLOR_STYLES: Record<StatoColor, { card: string; dot: string }> = {
...
const LEGEND: { color: StatoColor; label: string }[] = [
...
export interface PlannerCalendarProps {
  ...
  onOpen: (order: DtoOrderResponse) => void;
}

export default function PlannerCalendar({ orders, onOpen }: PlannerCalendarProps) {
  ...
  const ordersByDay = useMemo(() => {
    const buckets: DtoOrderResponse[][] = days.map(() => []);
    (orders || []).forEach(o => {
      ...
      <div className={`grid divide-x min-h-[320px] ${mode === 'day' ? 'grid-cols-1' : 'grid-cols-7'}`}>
        {ordersByDay.map((list, i) => (
          <div key={days[i].toISOString()} className={`flex flex-col gap-1.5 p-1.5 ${isToday(days[i]) ? 'bg-primary/5' : ''}`}>
            ...
            {list.map(o => {
              const color = (o.stato && COLOR_BY_STATO[o.stato]) || 'gray';
