...
import { getOrder, getDestinations, startOrder, closeOrder, discardOrder } from '@/lib/api';
...
import { ArrowLeft, Truck, PlayCircle, CheckCircle, Ban } from 'lucide-react';
...
export default function OrderDetailPage() {
  ...
  const fetchOrder = useCallback(() => {
    setLoading(true);
    getOrder(id).then(r => setOrder(r.data)).catch(err => { logger.error('Errore caricamento ordine:', err); toast.error('Ordine non trovato'); }).finally(() => setLoading(false));
  }, [id]);
  useEffect(() => { fetchOrder(); }, [fetchOrder]);

  useEffect(() => { getDestinations().then(r => setDestinations(r.data)).catch(err => logger.error('Errore caricamento destinazioni:', err)); }, []);
  ...
  const carico = order ? destById[order.destinazione_carico_id] : null;
  const scarico = order ? destById[order.destinazione_scarico_id] : null;
  ...
  return (
    <div className="space-y-3" data-testid="order-detail-page">
        ...
        <div className="ml-auto flex items-center gap-2">
          {order.stato === 'PIANIFICABILE' && (
            <>
              ...
              <Button onClick={() => setAssignDialogOpen(true)} data-testid="order-detail-assign">
                <Truck className="h-4 w-4 mr-2" /> Assegna
              </Button>
            </>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4 items-start">
        ...
        <Card className="rounded-xl border shadow-sm p-5 space-y-4">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-xl font-bold" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{order.cliente_nome}</h2>
              <StatusBadge stato={order.stato} />
            </div>
            <p className="text-sm text-muted-foreground mt-1">{order.destinazione_carico_nome} → {order.destinazione_scarico_nome}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-start gap-3">
              <div className="flex flex-col gap-1 w-36 shrink-0">
                ...
                <span className="text-sm font-bold leading-tight">{order.destinazione_carico_nome}</span>
              ...
              <div className="flex flex-col items-end gap-1 w-36 shrink-0 text-right">
                ...
                <span className="text-sm font-bold leading-tight">{order.destinazione_scarico_nome}</span>
        ...
        <Card className="rounded-xl border shadow-sm p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><p className="text-[10px] uppercase text-muted-foreground">Autista</p><p className="font-medium">{order.autista_nome || '—'}</p></div>
            <div><p className="text-[10px] uppercase text-muted-foreground">Vettore</p><p className="font-medium">{order.vettore_nome || '—'}</p></div>
          </div>
          {order.cliente_id && (
      ...
      <AssignOrderDialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen} order={order} onAssigned={fetchOrder} />
    </div>
  );
}
