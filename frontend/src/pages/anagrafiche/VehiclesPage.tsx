...
import type { DtoVehicleRequest, DtoVehicleResponse, DtoTemperatureReadingResponse } from '@/api/data-contracts';
...
const emptyForm: DtoVehicleRequest = { targa: '', tipo_veicolo: 'motrice', marca: '', modello: '', anno: 0, scompartature: 1, portata_kg: 0, note: '', gps_tracker_url: '', gps_tracker_tipo: '' };
...
export default function VehiclesPage() {
  ...
  const [form, setForm] = useState<DtoVehicleRequest>(emptyForm);
  ...
  const [tempDialog, setTempDialog] = useState<DtoVehicleResponse | null>(null);  // veicolo selezionato
  const [tempReadings, setTempReadings] = useState<DtoTemperatureReadingResponse[]>([]);
  const [tempLoading, setTempLoading] = useState(false);
  const [tempForm, setTempForm] = useState<TempForm>({ temp_min: '', temp_max: '' });
  const [tempSaving, setTempSaving] = useState(false);

  const openNew = () => { setForm(emptyForm); setEditId(null); setDialogOpen(true); };
  const openEdit = (item: DtoVehicleResponse) => { setForm({ targa: item.targa || '', tipo_veicolo: item.tipo_veicolo || 'motrice', marca: item.marca || '', modello: item.modello || '', anno: item.anno || 0, scompartature: item.scompartature || 1, portata_kg: item.portata_kg || 0, note: item.note || '', gps_tracker_url: item.gps_tracker_url || '', gps_tracker_tipo: item.gps_tracker_tipo || '' }); setEditId(item.id || null); setDialogOpen(true); };
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteVehicle(id).unwrap(); toast.success('Eliminato'); } catch (e) { toast.error(getMutationErrorMessage(e) || 'Errore'); }
  };

  const openTemp = async (vehicle: DtoVehicleResponse) => {
    setTempDialog(vehicle);
    setTempForm({
      temp_min: vehicle.temp_min != null ? String(vehicle.temp_min) : '',
      temp_max: vehicle.temp_max != null ? String(vehicle.temp_max) : '',
    });
    setTempReadings([]);
    setTempLoading(true);
    try {
  ...
  const handleSaveThresholds = async () => {
    ...
    setTempSaving(true);
    try {
      const payload: { temp_min?: number; temp_max?: number } = {};
      ...
      toast.error(getMutationErrorMessage(e) || 'Errore salvataggio soglie');
    } finally {
      setTempSaving(false);
    }
  };

  const tipoLabel: Record<string, string> = { motrice: 'Motrice', rimorchio: 'Rimorchio', rimorchio_isotermico: 'Rim. Isotermico', container: 'Container' };
      ...
      <DataTable columns={columns} data={data} loading={loading} searchValue={search} onSearchChange={setSearch} onAdd={openNew} addLabel="Nuovo Veicolo" testId="masterdata-table"
        renderRow={(item) => (
          <TableRow key={item.id} className="hover:bg-muted/60">
            ...
            <TableCell className="py-2">{(item.tipo_veicolo && tipoLabel[item.tipo_veicolo]) || item.tipo_veicolo}</TableCell>
                  ...
                  {tempLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={[...tempReadings].reverse().map(r => ({
                        ...
                        <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} formatter={(v: number) => `${v.toFixed(1)} °C`} />
