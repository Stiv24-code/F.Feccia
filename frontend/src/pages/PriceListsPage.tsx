...
const emptyRule: DtoPriceListItemRequestDTO = {
  prodotto_id: '',
...
const emptyForm: DtoPriceListRequest = { cliente_id: '', data_inizio: '', data_fine: '', items: [], note: '' };

export default function PriceListsPage() {
  const [lists, setLists] = useState<DtoPriceListResponse[]>([]);
  ...
  const [ruleForm, setRuleForm] = useState<DtoPriceListItemRequestDTO>(emptyRule);
  const [editingItemId, setEditingItemId] = useState<string | null>(null);
  const [savingRule, setSavingRule] = useState(false);

  const fetchData = useCallback(() => {
    setLoading(true);
    getPriceLists().then((r: { data: DtoPriceListResponse[] }) => setLists(r.data)).catch((err: unknown) => logger.error('Errore caricamento listini:', err)).finally(() => setLoading(false));
  }, []);
  ...
  const setCustomer = (id: string) => setForm({ ...form, cliente_id: id });
  ...
  // --- Dettaglio ---
  const openDetail = async (listino: DtoPriceListResponse) => {
    if (!listino.id) return;
    try {
  ...
  const openEditRule = (item: DtoPriceListItemResponseDTO) => {
    setRuleForm({
  ...
  const handleSaveRule = async () => {
    if ((ruleForm.tariffa || 0) <= 0) { toast.error('Inserire una tariffa valida'); return; }
    if (!selectedList?.id) return;
    setSavingRule(true);
    try {
      ...
      await refreshDetail();
    } catch (e) { toast.error(getApiErrorMessage(e) || 'Errore'); } finally { setSavingRule(false); }
  };

  const handleDeleteRule = async (itemId?: string) => {
    if (!itemId || !selectedList?.id) return;
  ...
  const setRuleCarico = (id: string) => setRuleForm({ ...ruleForm, destinazione_carico_id: id });
  const setRuleScarico = (id: string) => setRuleForm({ ...ruleForm, destinazione_scarico_id: id });
  const setRuleProdotto = (id: string) => setRuleForm({ ...ruleForm, prodotto_id: id });
              ...
              <TableBody>
                {(!selectedList.items || selectedList.items.length === 0) ? (
                  ...
                  <TableRow key={item.item_id} className="hover:bg-muted/60">
                    ...
                    <TableCell className="py-2 text-right tabular-nums font-medium">€ {formatEuro(item.tariffa || 0)}</TableCell>
                    <TableCell className="py-2">
                    ...
                    <TableCell className="py-2 tabular-nums">
                      {((item.range_peso_min || 0) > 0 || (item.range_peso_max || 0) > 0)
                        ? `${formatEuro(item.range_peso_min || 0)} - ${formatEuro(item.range_peso_max || 0)} ${item.unita_peso}`
                        : <span className="text-muted-foreground">—</span>}
                    </TableCell>
                    <TableCell className="py-2 text-right tabular-nums">
                      {(item.minimo_tassabile || 0) > 0 ? `${formatEuro(item.minimo_tassabile || 0)} ${item.unita_peso}` : '—'}
                    </TableCell>
                    <TableCell className="py-2">
                      <Badge variant="outline" className="text-[10px]">{item.tipo_trasporto === 'intermodale' ? 'Intermod.' : 'Stradale'}</Badge>
                    </TableCell>
                    <TableCell className="py-2 text-right tabular-nums">
                      {(item.perc_adeguamento_carburante || 0) > 0 ? `${item.perc_adeguamento_carburante}%` : '—'}
                    </TableCell>
                    <TableCell className="py-2">
