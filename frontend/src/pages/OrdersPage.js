...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(() => {
    ...
    getOrders({ stato: statoFilter === 'all' ? '' : statoFilter, search, tipologia: tipologiaFilter === 'all' ? '' : tipologiaFilter })
      ...
      .catch(err => logger.error('Errore caricamento ordini:', err))
  ...
  const openReturns = async (order) => {
    ...
    try {
      ...
      toast.error(e.response?.data?.detail || 'Errore caricamento ritorni');
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteOrder(id); toast.success('Eliminato'); fetchOrders(); } catch(e) { toast.error(e.response?.data?.detail || 'Errore'); }
  };
  ...
  const tryLookupTariff = async (clienteId, caricoId, scaricoId) => {
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast.error(e.response?.data?.detail || 'Errore download CMR');
                            }
                          }}
                        >
