    ...
    <div className="space-y-4 lg:space-y-6" data-testid="customer-dashboard-page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <KPICard
          ...
          icon={ClipboardList}
          description={`${kpi.ordini_pianificabili} da pianificare`}
        />
        <KPICard
          title="In Viaggio"
          value={kpi.ordini_in_viaggio}
          icon={Layers}
          description={`${kpi.ordini_chiusi} chiusi`}
        />
        <KPICard
          title="Fatturati"
          value={kpi.ordini_fatturati}
          icon={FileText}
          description={`Tariffa media € ${formatEuro(kpi.tariffa_media)}`}
        />
        <KPICard
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={monthly_trend}>
                    ...
                    <Tooltip
                      ...
                      formatter={(val, name) => name === 'fatturato' ? [`€ ${formatEuro(val)}`, 'Fatturato'] : [val, 'Ordini']}
                    />
                ...
                {top_destinazioni.map((d, i) => (
                  <li key={d.nome} className="flex items-center justify-between gap-2 py-1 border-b last:border-b-0">
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums whitespace-nowrap">
                      {d.ordini} ord · € {formatEuro(d.fatturato)}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <CardContent>
            <div className="h-48">
              {per_tipologia.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={per_tipologia} dataKey="ordini" nameKey="tipologia" outerRadius={70} label={(e) => `${e.tipologia}: ${e.ordini}`}>
