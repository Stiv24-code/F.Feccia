...
export default function InvoicesPage() {
  ...
  const fetchInvoices = useCallback(() => { setLoading(true); getInvoices().then(r => setInvoices(r.data)).catch(err => logger.error('Errore caricamento fatture:', err)).finally(() => setLoading(false)); }, []);
  ...
  const selectedTotal = useMemo(() => closedOrders.filter(o => selectedOrderIds.includes(o.id)).reduce((s, o) => s + (o.tariffa || 0), 0), [closedOrders, selectedOrderIds]);
  ...
  const handleDeleteInvoice = async (id) => {
    ...
    try { await deleteInvoice(id); toast.success('Eliminata'); fetchInvoices(); } catch(e) { toast.error(e.response?.data?.detail || 'Errore'); }
  };
  ...
  return (
    <div className="space-y-3" data-testid="invoices-page">
      <div className="flex justify-between items-center">
        <Tabs value={tab} onValueChange={setTab} data-testid="invoicing-tabs">
          <TabsList>
          ...
          {selectedInvoice && (
            <div className="space-y-3 text-sm">
                  ...
                  <div className="border rounded-lg divide-y">
                    {selectedInvoice.righe.map((r, i) => (
                      <div key={r.ordine_id || `riga-${i}`} className="px-3 py-2 flex justify-between">
                        ...
                        <span className="tabular-nums font-medium">€ {formatEuro(r.totale)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex justify-between font-medium text-base pt-2 border-t">
                <span>Totale:</span>
                <span>€ {formatEuro(selectedInvoice.totale)}</span>
              </div>
